...
const videoboxStripContainerCss = "/*!@:host*/.sc-videobox-strip-container-h{position:absolute}/*!@.wrapper*/.wrapper.sc-videobox-strip-container{display:flex;flex-direction:row-reverse;gap:10px}";
...
class VideoboxStripContainer {
  ...
  render() {
    ...
    return (hAsync(Host, null, hAsync("div", { class: "wrapper" }, data.map((item, index) => hAsync("videobox-strip-item", { item: item, index: index })))));
  }
...
class VideoboxStripItem {
  ...
  static get cmpMeta() { return {
    "$flags$": 9,
    "$tagName$": "videobox-strip-item",
    "$members$": {
      "item": [16],
      "index": [2],
      "active": [32]
    },
    "$listeners$": [[1, "mouseenter", "mouseEnterHandler"], [1, "mouseleave", "mouseLeaveHandler"]],
    "$lazyBundleId$": "-",
    "$attrsToReflect$": []
  }; }
}
...
class VideoboxVideo {
  ...
  render() {
    return (hAsync(Host, null, hAsync("video", { src: this.src, muted: true, loop: true, ref: el => this.videoRef = el })));
  }
